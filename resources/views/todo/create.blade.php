                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('todo.store') }}" class="mt-6 space-y-6">
                        ...
                        <div>
                            ...
                            <x-textarea id="description" class="block w-full mt-1" name="description" rows="3">{{ old('description') }}</x-textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Save') }}</x-primary-button>
                            <x-secondary-button href="{{ route('todo.index') }}">{{ __('Cancel') }}</x-secondary-button>
                        </div>
                    </form>
                </div>
            </div>
